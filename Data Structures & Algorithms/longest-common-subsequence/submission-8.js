class Solution {
    ...
    helper(text1, text2, index1, index2, memo) {
        ...
        if (text1.at(index1) === text2.at(index2)) {
            result = Math.max(result, 1 + this.helper(text1, text2, index1 + 1, index2 + 1, memo));
        } else {
            result = Math.max(this.helper(text1, text2, index1, index2 + 1, memo), this.helper(text1, text2, index1 + 1, index2, memo));
        }
