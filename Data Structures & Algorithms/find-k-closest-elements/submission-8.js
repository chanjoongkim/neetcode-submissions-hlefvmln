class Solution {
    ...
    findClosestElements(arr, k, x) {
        // O(log N) time for this portion
        const closestIndex = this.modBinarySearch(arr, x);
        
        let l = closestIndex - 1;
        let r = closestIndex;

        // O(K) time for this portion
        // build window while it is less than k, starting at the "closest" index from our modified binary search
        while ((r - l - 1) < k) {
            // boundary checks
            if (l < 0) {
                r++;
                ...
                // compare left and right ends of window
