class Solution {
    ...
    public int largestRectangleArea(int[] heights) {
        ...
        for (int i = 0; i < heights.length; i++) {
            ...
            // if (!stack.isEmpty() && currHeight <= stack.peek().height) {
            //     Height oldTop = stack.pop();
            //     Height newTop = new Height(currHeight, oldTop.index);
            //     stack.push(newTop);
            //     largestArea = Math.max(largestArea, currHeight * (i - oldTop.index + 1));
            // } else {
            //     // check if max could be continuation of old height with current index
            //     if (!stack.isEmpty()) {
            //         Height currTop = stack.peek();
            //         System.out.println("currTop " + currTop + " i " + i);
            //         largestArea = Math.max(largestArea, (i - currTop.index + 1) * currTop.height);
            //     }
            //     Height newTop = new Height(currHeight, i);
            //     stack.push(newTop);
            // }
        }
